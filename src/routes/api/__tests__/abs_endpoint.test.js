...
describe('ABS endpoint tests', () => {
        ...
        it('post a submission via: /api/abs/submit', async (done) => {
            ...
            await request(app)
                ...
                .then(response => {
                    ...
                    expect(response.body.period_of_observation_from.toString()).toMatch("2020-01-17T14:00:00.000Z");
                    expect(response.body.period_of_observation_to.toString()).toMatch("2020-01-19T14:00:00.000Z");
    ...
    describe("ABS get request endpoints", () => {
                ...
                .then(async response => {
                    await request(app)
                        ...
                        .then(response2 => {
                            ...
                            expect(submission.period_of_observation_from.toString()).toMatch("2020-01-17T14:00:00.000Z");
                            expect(submission.period_of_observation_to.toString()).toMatch("2020-01-19T14:00:00.000Z");
        ...
        it('get all patient submissions via: /api/abs/submissions/:patientId', async (done) => {
            const answers = shift => [...Array(14).keys()].map(key=>({
                questionNum: key+1,
                score: (key+1+shift) % 4,
            }));
